<template>
  <mainlayout>
    <div class="query-model-container">
      <h1 class="title">查詢機型</h1>
      <div class="form-container">
        <form @submit.prevent="submitForm" class="model-form">
          <div class="form-group">
            <label for="model_id">機型編號</label>
            <input
              type="text"
              id="model_id"
              v-model="formData.model_id"
              class="form-input"
              :class="{ error: errors.model_id }"
              placeholder="請輸入機型編號"
            />
            <span class="error-message" v-if="errors.model_id">
              {{ errors.model_id }}
            </span>
          </div>
          ...
          <div class="form-hint">* 可輸入機型編號或名稱進行查詢</div>

          <div class="button-group">
            <button type="submit" class="submit-btn">查詢機型</button>
            <button type="button" class="reset-btn" @click="resetForm">
              重置
            </button>
            <button type="button" class="query-all-btn" @click="queryAllModels">
              查詢全部
            </button>
          ...
          >
            <div class="model-info">
              <div class="info-section">
                <p><strong>機型編號:</strong> {{ model.model_id }}</p>
                <p><strong>機型名稱:</strong> {{ model.modelname }}</p>
                <p>
                  <strong>狀態:</strong>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form @submit.prevent="submitAddModel" class="add-form">
            <div class="form-group">
              <label>品牌</label>
              <select v-model="addForm.brand" class="form-input">
            ...
            <div class="form-group">
              ...
              <input
                ...
                class="form-input"
                placeholder="輸入格式為:Galaxy S24 Ultra、iPhone XX Pro Max"
                required
              />
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                class="form-input"
                required
                placeholder="輸入格式為:Samsung品牌、Apple品牌"
              ></textarea>
            </div>
...
export default {
  ...
  computed: {
    ...
    maxDate() {
      const today = new Date();
      today.setDate(today.getDate() + 90); // 允許選擇未來 90 天的日期
      return today.toISOString().split("T")[0];
  ...
  methods: {
      ...
      try {
        ...
        if (response.status === 200 && response.data.success) {
          ...
          this.brands.forEach((brand) => {
            this.brandMap[brand.name] = brand.brand_id;
          });
        } else {
    ...
    validateForm() {
      ...
      if (!this.formData.modelname.trim() && !this.formData.model_id.trim()) {
    ...
    async submitForm() {
      ...
      try {
        ...
        if (this.formData.modelname.trim()) {
          params.name = this.formData.modelname.trim(); // 確保參數名稱與後端一致
        }
        if (this.formData.model_id.trim()) {
          params.model_id = this.formData.model_id.trim();
        }

        const response = await api.get("/products/getmodelname", { params });
        ...
        if (response.status === 200 && response.data.modelDetails) {
          this.modelList = Array.isArray(response.data.modelDetails)
            ...
            : [response.data.modelDetails];
          console.log("Model List:", this.modelList); // 檢查 modelList 的內容
        } else {
          this.modelList = [];
        }
      } catch (error) {
    ...
    validateEditForm() {
      ...
      if (
        ...
        this.editErrors.is_active = "請選擇機型狀態";
        isValid = false;
      }
    ...
    async submitEdit() {
      ...
      try {
        // 這裡修正了 submitData 的定義
        const submitData = {
          model_id: this.editForm.model_id,
          is_active: Number(this.editForm.is_active),
        };

        console.log("Submitting data:", submitData);

        // 修正 API 路徑
        const response = await api.put("/products/updatemodels", submitData);

        if (response.status === 200) {
          // 根據當前的查詢條件重新加載數據
          if (this.formData.modelname || this.formData.model_id) {
            await this.submitForm();
          } else {
            await this.queryAllModels();
          }
    ...
    async submitAddModel() {
      try {
        // 將品牌名稱轉換為對應的 brand_id
        const brand_id = this.brandMap[this.addForm.brand];

        if (!brand_id) {
          throw new Error("無效的品牌選擇");
        }
        ...
        const response = await api.post("/products/addmodels", submitData);
